...
const ESSENTIAL_INFO = [
  { 
    id: 'emergency', title: 'Emergency Hotlines', subtitle: 'Campus Security & Medical', icon: 'phone-call', IconComponent: Feather, bgColor: '#ffebee', iconColor: '#d32f2f',
    details: '🚨 Campus Security: (062) 123-4567\n🚑 Medical Emergency: (062) 987-6543\nAvailable 24/7 for all students and staff.' 
  },
  { 
    id: 'registrar', title: 'Registrar Services', subtitle: 'Grades, clearances, documents', icon: 'file-document-outline', IconComponent: MaterialCommunityIcons, bgColor: '#e3f2fd', iconColor: '#1976d2',
    details: '📍 Located at the Main Administration Building, 1st Floor.\n🕒 Office Hours: Mon-Fri, 8:00 AM - 5:00 PM.\nEmail: registrar@msu.edu.ph' 
  },
  { 
    id: 'it_help', title: 'IT Helpdesk', subtitle: 'Portal & WiFi assistance', icon: 'headset-mic', IconComponent: MaterialIcons, bgColor: '#fff8e1', iconColor: '#ffa000',
    details: 'Need help logging into your portal or connecting to campus WiFi?\nSubmit a ticket at ithelp.msu.edu.ph or visit the ICT Office.' 
  },
  { 
    id: 'clinic', title: 'University Clinic', subtitle: 'Medical records & consultations', icon: 'medical-bag', IconComponent: MaterialCommunityIcons, bgColor: '#e8f5e9', iconColor: '#388e3c',
    details: 'Free basic consultations for enrolled students. Please bring your valid University ID. Dental services are available by appointment.' 
  },
];
...
const AnimatedGridCard = ({ item, onPress }: any) => {
  ...
  const handlePressOut = () => Animated.spring(scale, { toValue: 1, bounciness: 12, useNativeDriver: true }).start();

  return (
    <TouchableOpacity activeOpacity={0.9} onPressIn={handlePressIn} onPressOut={handlePressOut} onPress={onPress} style={styles.cardWrapper}>
...
export default function Dashboard() {
  ...
  const handleHandbookPressIn = () => Animated.spring(handbookScale, { toValue: 0.95, useNativeDriver: true }).start();
  const handleHandbookPressOut = () => Animated.spring(handbookScale, { toValue: 1, bounciness: 12, useNativeDriver: true }).start();
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={[styles.content, { paddingBottom: 120 }]} showsVerticalScrollIndicator={false}>
        ...
        <TouchableOpacity activeOpacity={0.9} onPressIn={handleHandbookPressIn} onPressOut={handleHandbookPressOut} onPress={() => console.log("Redirect")}>
          <Animated.View style={[styles.handbook, { transform: [{ scale: handbookScale }] }]}>
            <View>
        ...
        <View style={styles.grid}>
          {GRID_ITEMS.map((item) => (
            <AnimatedGridCard key={item.key} item={item} onPress={() => console.log(`Redirect to ${item.key}`)} />
          ))}
        </View>
            ...
            return (
              ...
              >
                ...
                <View style={styles.listCardHeader}>
                  <View style={[styles.listIconWrapper, { backgroundColor: info.bgColor }]}>
                    <IconComp name={info.icon as any} size={22} color={info.iconColor} />
                  </View>
                  ...
                  <Entypo name={isExpanded ? "chevron-small-up" : "chevron-small-down"} size={24} color="#888" />
                </View>
...
const styles = StyleSheet.create({
  ...
  appName: { fontSize: 12, color: "#D4AF37", fontWeight: "700", textTransform: 'uppercase', letterSpacing: 1, marginBottom: 4 },
  ...
  card: { 
    backgroundColor: "#ffffff", borderWidth: 1.5, borderColor: "#E5E5E5", 
    padding: 20, alignItems: "center", borderRadius: 16, height: 140, justifyContent: "center",
  ...
  sectionTitle: { fontSize: 18, color: "#4A0E0E", fontWeight: "700", marginBottom: 15, marginLeft: 5 },
  listCard: { 
    ...
    borderWidth: 1.5, 
    borderColor: "#E5E5E5",
    shadowColor: "#800505", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.04, shadowRadius: 4, elevation: 2,
  },
  listCardHeader: { flexDirection: 'row', alignItems: 'center' },
  listIconWrapper: { width: 48, height: 48, borderRadius: 12, justifyContent: 'center', alignItems: 'center', marginRight: 16 },
  listTextContainer: { flex: 1 },
  listTitle: { fontSize: 16, color: "#4A0E0E", fontWeight: "700", marginBottom: 3 },
